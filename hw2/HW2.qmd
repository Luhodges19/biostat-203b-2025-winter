...
```{r}
system.time({
  labevents.filter.arrow <- arrow::open_dataset("labevents.csv", format = "csv") %>%
    dplyr::select(subject_id, itemid, charttime, valuenum) %>%
    dplyr::filter(itemid %in% c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931))
})
```
...
**The Parquet file is 2.7 GB before being selected/filtered out**
...
**Now, let us see how large the file size is now that it is filtered**
...
```{r}
nrow(
  collect(labevents.filter.duckdb)
  )
```
...
**DuckDB, like Parquet and Arrow, utilizes a columnar based format which allows for easier access and storage of the data rather than using rows. It can read both Parquet and Arrow without loading the entire file into memory, which is beneficial for big data applications as it allows for faster turn around.**
