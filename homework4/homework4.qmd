...
#For the dod, we need to do it in two separate parts to show the median and the range. Here is the first part. 
#This is going to be getting what we need for the beginning part. The latter half will be done by the code itself. 
...
mimic_icu_cohort2 <- mimic_icu_cohort |> 
  ...
  mutate(
    ...
    admission_type = fct_lump_n(factor(admission_type), n = 4, 
                                other_level = "Other"),
    admission_location = fct_lump_n(factor(admission_location), n = 4, 
                                    other_level = "Other"),
    ...
    race = case_when(
      ...
      TRUE ~ "Other"),
    race = factor(race, levels = c("ASIAN", "BLACK", 
